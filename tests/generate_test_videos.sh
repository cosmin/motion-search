...
ffmpeg -y -f lavfi -i testsrc=size=${WIDTH}x${HEIGHT}:rate=${FRAMERATE} \
  -pix_fmt yuv420p -frames:v ${FRAMES} "${TEST_DATA_DIR}/testsrc.yuv"

# 2. SMPTE color bars (static)
echo "  - smptebars.yuv (color bars - static)"
ffmpeg -y -f lavfi -i smptebars=size=${WIDTH}x${HEIGHT}:rate=${FRAMERATE} \
...
ffmpeg -y -f lavfi -i color=c=black:size=${WIDTH}x${HEIGHT}:rate=${FRAMERATE} \
...
ffmpeg -y -f lavfi -i color=c=white:size=${WIDTH}x${HEIGHT}:rate=${FRAMERATE} \
  -pix_fmt yuv420p -frames:v ${FRAMES} "${TEST_DATA_DIR}/white.yuv"

# 5. Gray pattern (mid-range values)
echo "  - gray.yuv (50% gray - no motion)"
ffmpeg -y -f lavfi -i color=c=gray:size=${WIDTH}x${HEIGHT}:rate=${FRAMERATE} \
...
# 9. Moving box (known motion vector) - simplified
echo "  - moving_box.yuv (white box moving right)"
ffmpeg -y -f lavfi -i "color=c=black:s=${WIDTH}x${HEIGHT}:r=${FRAMERATE},format=yuv420p[bg];color=c=white:s=32x32:r=${FRAMERATE}[box];[bg][box]overlay=x='t*30':y=${HEIGHT}/2-16" \
...
ffmpeg -y -f lavfi -i testsrc=size=${WIDTH}x${HEIGHT}:rate=1 \
...
ffmpeg -y -f lavfi -i color=c=blue:size=${WIDTH}x${HEIGHT}:rate=1 \
