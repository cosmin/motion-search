 ...
 * Validates that SSE2 optimized versions produce identical results to reference C implementations
 */
...
protected:
    ...
    void fillCheckerboard(uint8_t* data, int width, int height, int stride, int blockSize = 8) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                data[y * stride + x] = ((x / blockSize) + (y / blockSize)) % 2 ? 255 : 0;
            }
        }
    }
...
TEST_F(MomentsTest, SAD8_RandomData) {
    ...
    int sad_c = fastSAD8_c(current.data(), reference.data(), stride,
                           block_width, block_height, INT32_MAX);
    int sad_opt = fastSAD8(current.data(), reference.data(), stride,
                           block_width, block_height, INT32_MAX);
...
TEST_F(MomentsTest, SAD4_RandomData) {
    ...
    int sad_c = fastSAD4_c(current.data(), reference.data(), stride,
                           block_width, block_height, INT32_MAX);
    int sad_opt = fastSAD4(current.data(), reference.data(), stride,
                           block_width, block_height, INT32_MAX);
...
TEST_F(MomentsTest, SAD16_IdenticalBlocks) {
    ...
    int sad_c = fastSAD16_c(data.data(), data.data(), stride,
                            block_width, block_height, INT32_MAX);
    int sad_opt = fastSAD16(data.data(), data.data(), stride,
                            block_width, block_height, INT32_MAX);
...
TEST_F(MomentsTest, SAD16_MaxDifference) {
    ...
    int sad_c = fastSAD16_c(black.data(), white.data(), stride,
                            block_width, block_height, INT32_MAX);
    int sad_opt = fastSAD16(black.data(), white.data(), stride,
                            block_width, block_height, INT32_MAX);
...
TEST_F(MomentsTest, SAD_EarlyTermination) {
    ...
    int sad_c = fastSAD16_c(black.data(), white.data(), stride,
                            block_width, block_height, min_SAD);
    int sad_opt = fastSAD16(black.data(), white.data(), stride,
                            block_width, block_height, min_SAD);
...
TEST_F(MomentsTest, Variance16_RandomData) {
    ...
    EXPECT_EQ(var_c, var_opt) << "Variance16 optimized version differs from reference";
}
...
TEST_F(MomentsTest, Variance8_RandomData) {
    ...
    EXPECT_EQ(var_c, var_opt) << "Variance8 optimized version differs from reference";
}
...
TEST_F(MomentsTest, Variance4_RandomData) {
    ...
    EXPECT_EQ(var_c, var_opt) << "Variance4 optimized version differs from reference";
}
...
TEST_F(MomentsTest, MSE16_IdenticalBlocks) {
    ...
    int mse_c = fast_calc_mse16_c(data.data(), data.data(), stride,
                                  block_width, block_height);
    int mse_opt = fast_calc_mse16(data.data(), data.data(), stride,
                                  block_width, block_height);
...
TEST_F(MomentsTest, BidirMSE16_RandomData) {
    ...
    EXPECT_EQ(mse_c, mse_opt) << "BidirMSE16 optimized version differs from reference";
}
...
TEST_F(MomentsTest, BidirMSE8_RandomData) {
    ...
    EXPECT_EQ(mse_c, mse_opt) << "BidirMSE8 optimized version differs from reference";
}
...
TEST_F(MomentsTest, BidirMSE4_RandomData) {
    ...
    EXPECT_EQ(mse_c, mse_opt) << "BidirMSE4 optimized version differs from reference";
}
