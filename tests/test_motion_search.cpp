...
TEST_F(MotionSearchTest, SpatialSearch_IdenticalFrames) {
  ...
  int result = spatial_search(center, center, stride, dim, block_width,
                              block_height, motion_vectors.data(), SADs.data(),
                              mses.data(), MB_modes.data(), &count_I, &bits);
  ...
  for (int y = 0; y < num_blocks_y; y++) {
    for (int x = 0; x < num_blocks_x; x++) {
      ...
      EXPECT_EQ(0, mses[idx])
          << "MSE should be 0 for identical frames at block " << idx;
    }
  }
}
...
TEST_F(MotionSearchTest, SpatialSearch_DifferentContent) {
  ...
  int result = spatial_search(current, reference, stride, dim, block_width,
                              block_height, motion_vectors.data(), SADs.data(),
                              mses.data(), MB_modes.data(), &count_I, &bits);
  ...
  for (int y = 0; y < num_blocks_y; y++) {
    for (int x = 0; x < num_blocks_x; x++) {
      int idx = y * stride_MB + x;
      EXPECT_GT(mses[idx], 0)
          << "MSE should be > 0 for different frames at block " << idx;
    }
  }
}
...
TEST_F(MotionSearchTest, MotionSearch_KnownMotion) {
  ...
  for (int y = square_y + shift_y;
       y < square_y + square_size + shift_y && y < height; y++) {
    for (int x = square_x + shift_x;
         x < square_x + square_size + shift_x && x < width; x++) {
...
TEST_F(MotionSearchTest, MotionSearch_MultipleBlockSizes) {
  ...
  EXPECT_EQ(0, width % block_width)
      << "Width should be divisible by block width";
  EXPECT_EQ(0, height % block_height)
      << "Height should be divisible by block height";
