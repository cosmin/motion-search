...
TEST_F(FrameTest, ExtendFrame_BasicFunctionality) {
    ...
    extend_frame(frame.data() + pad_y * stride + pad_x, stride, dim, pad_x, pad_y);

    // Verify top border - should replicate first row
    // The top padding rows should be copies of the first row (including left/right padding)
    for (int y = 0; y < pad_y; y++) {
        for (int x = 0; x < stride; x++) {
            EXPECT_EQ(frame[pad_y * stride + x], frame[y * stride + x])
                << "Top padding row " << y << " col " << x << " not replicated correctly";
        }
    }
    ...
    for (int y = 0; y < total_height; y++) {
        for (int x = 0; x < stride; x++) {
            EXPECT_EQ(128, frame[y * stride + x])
                << "Extended pixel at (" << x << ", " << y << ") incorrect for constant image";
        }
    }
}
